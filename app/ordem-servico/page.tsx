  ...
  return (
    <div className="min-h-screen bg-gray-100 p-2 md:p-6 font-sans text-gray-900">
      <div className="max-w-2xl mx-auto">
        <div className="mb-4">
          <Link href="/" className="text-sm text-blue-600 hover:text-blue-800">&larr; Voltar ao Dashboard</Link>
        </div>
        <div className="bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-200">
          <header className="bg-gray-800 p-6 text-white flex justify-between items-center">
            <h1 className="text-2xl font-black text-white italic leading-none">SOUMOTOS - O.S.</h1>
          ...
          <main className="p-4 md:p-8 space-y-8">
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                { label: "Mecânico Responsável", key: "mecanico" as const, extra: "bg-blue-50 font-bold col-span-1 md:col-span-2" }
              ].map((field) => (
                <div key={field.key} className={`flex flex-col gap-1 ${field.key === 'mecanico' ? 'md:col-span-2' : ''}`}>
                  <label className="text-[10px] font-black text-gray-500 ml-1 uppercase">{field.label}</label>
                  <input
            ...
            <section className="bg-gray-50 p-4 rounded-2xl border border-gray-200">
              <h2 className="font-black text-sm mb-4 border-l-4 border-blue-500 pl-2 text-gray-700 uppercase italic">🔍 Inspeção Visual Externa</h2>
              <div className="grid grid-cols-1 gap-2">
                {checklistItems.map(item => (
                  <div key={item} className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm transition-all hover:border-blue-200">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={checklist[item]?.marked || false} className="w-6 h-6 rounded-md border-gray-300 text-blue-600 focus:ring-blue-500" onChange={() => handleCheckChange(item)} />
                      <span className="text-sm font-bold text-gray-700">{item}</span>
                    </label>
                    {checklist[item]?.marked && (
                      <div className="mt-2 flex flex-col gap-1">
                        <label className="text-[9px] font-bold text-blue-700 ml-1 uppercase">Detalhes da avaria em {item}</label>
                        <input value={checklist[item]?.obs || ''} placeholder="Ex: Risco profundo..." className="w-full text-xs p-2 bg-blue-50 border border-blue-100 rounded-md outline-none italic" onChange={e => handleCheckObs(item, e.target.value)} />
                      </div>
                    )}
                  </div>
                ))}
            ...
            <section className="space-y-6">
               {[
                 ...
                 { label: "Observações Gerais da Oficina", key: "obsGerais" as const, color: "text-gray-500" },
                 { label: "Serviços (Executados)", key: "servicosExecutados" as const, color: "text-green-600", bg: "bg-green-50/30" },
                 { label: "Parecer Técnico", key: "parecer" as const, color: "text-gray-700" },
                 { label: "Peças e Lubrificantes", key: "pecas" as const, color: "text-gray-700" }
               ].map((area) => (
          ...
          <footer className="p-4 md:p-6 sticky bottom-0 bg-white/90 backdrop-blur-md border-t flex flex-col gap-3">
            <button onClick={copiarTexto} className={`w-full py-4 rounded-xl font-bold text-xs transition-all ${copiado ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
